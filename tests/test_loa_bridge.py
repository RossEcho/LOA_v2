...
class TestLoaBridge(unittest.TestCase):
    ...
    @patch("src.loa_bridge.snapshot", return_value="abc123")
    @patch("src.loa_bridge.subprocess.run")
    def test_dispatch_onboarded_maps_nmap_ping_scan_type_to_sn(
        self, run_mock, snapshot_mock, rollback_mock, _registry_mock, _spec_mock
    ):
        run_mock.return_value = subprocess.CompletedProcess(
            args=["/usr/bin/nmap", "-sn", "192.168.7.3"],
            returncode=0,
            stdout="ok",
            stderr="",
        )
        result = _dispatch_tool(
            {
                "tool_name": "nmap",
                "args": {"scan_type": "ping", "target": "192.168.7.3"},
                "cwd": None,
                "timeout_seconds": 3,
                "action_class": "SYSTEM",
                "env": None,
        ...
        self.assertTrue(result["ok"])
        called_argv = run_mock.call_args[0][0]
        self.assertIn("-sn", called_argv)
        self.assertNotIn("ping", called_argv)
        rollback_mock.assert_not_called()
        snapshot_mock.assert_called_once()
